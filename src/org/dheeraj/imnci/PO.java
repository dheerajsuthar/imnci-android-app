...
public class PO extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		String tableName = "po";
		String labels[] = { "ID", "place of delivery", "delivery type",
				"complication", "date of delivery", "date of discharge",
				"JSY Benefits" };
		String places_of_del[] = { "SBA", "NSBA", "HSC", "PHC", "CHC", "SDH",
				"DH", "P" };
		String types_of_delivery[] = { "N", "C", "I" };
		String complications[] = { "Y", "N" };
		String jsy_benefits[] = { "Y", "N" };
		...
		for (Data data : dataMap) {
			...
			if (data.getLabel().equals("place of delivery")) {
				showSpinner(data, places_of_del);
			} else if (data.getLabel().equals("delivery type")) {
				showSpinner(data, types_of_delivery);
			} else if (data.getLabel().equals("complication")) {
				showSpinner(data, complications);
			} else if (data.getLabel().equals("JSY Benefits")) {
				showSpinner(data, jsy_benefits);
			} else if (data.getLabel().equals("date of delivery")) {
				showDateDialog(data);
			} else if (data.getLabel().equals("date of discharge")) {
				showDateDialog(data);
	...
	private String[] getMidArray() {
		...
		try {
			...
			String column[] = { "mid" };
			cursor = dbReader.query("mother_reg", column, null, null, null,
					null, "mid desc");
	...
	void showDateDialog(Data data) {
		...
		dateButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				dobDialog = new DatePickerDialog(PO.this, l, c
